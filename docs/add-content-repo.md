...
Astro 会自动处理 markdown 里的**相对路径图片**（转 webp、懒加载、补 `/blog` 前缀），
无需任何配置。要让图片显示到网页，遵守 3 条：

1. 图片存进**同一个笔记仓库**（如 `<NOTES_DIR>/images/foo.png`），和 md 一起 commit、push。
2. 用**相对路径**引用（相对 md 文件位置）：`![说明](images/foo.png)`。
3. 路径用正斜杠 `/`。

不会显示的写法：
- 绝对路径 `C:\Users\...\foo.png`（只在本机有效）
- 以 `/` 开头 `/images/foo.png`（指向 `public/`，不是笔记仓库）
- 图片存在仓库外（如 Typora 全局图片文件夹，CI 上没有该文件）

> 用 Typora：设置 → 图像 → 「复制图片到」相对文件夹（如 `./${filename}.assets`），
> 图片会自动存到 md 旁边、跟着进仓库。
